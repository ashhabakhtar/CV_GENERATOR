...
export const ExperienceForm: React.FC<ExperienceFormProps> = ({ data, update }) => {
  ...
  const isDateInvalid = (startDate: string, endDate: string, isCurrent: boolean) => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">List your professional journey. <span className="text-red-400">*</span> Required fields.</p>
        </div>
        <button
      ...
      <div className="space-y-4">
        {data.map((exp) => {
          const dateError = isDateInvalid(exp.startDate, exp.endDate, exp.current);
          
          return (
            <div key={exp.id} className={`glass-card p-6 border transition-all relative group ${dateError ? 'border-red-500/50 bg-red-500/5' : 'border-white/5'}`}>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Company <span className="text-red-400">*</span></label>
                  <input
                    type="text"
                    required
                    value={exp.company}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Position <span className="text-red-400">*</span></label>
                  <input
                    type="text"
                    required
                    value={exp.position}
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className={`w-full bg-slate-950/50 border rounded-lg px-3 py-2 text-sm text-white focus:outline-none transition-all disabled:opacity-50 ${dateError ? 'border-red-500/50 ring-1 ring-red-500/20' : 'border-white/10 focus:border-primary-500/50'}`}
                  />
                </div>
              </div>
